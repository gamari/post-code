# 概要

タスクやデザインをまとめます

## TODO

- [ ] AIによる記事セクションを作成する

- [ ] AIコメント機能
  - AI-ID props

- [ ] ファイル引用 同一ファイルがある場合の挙動を確認する

- [ ] .envの追加
  - API_PASSWORD
  - AI_ID

- [ ] アマギフプレゼント企画
- [ ] エラーページを作成する
- [ ] トップページのOGPなどを修正
- [ ] はじめてのアウトプット記事を作成する

- [ ] CodeSectionでまとめられる

- [ ] コメント
  - [ ] 返信機能
  - [x] 順番を降順にする
    - やめる

- [ ] 投稿ツイート作成API（タグをつける）
  - [x] 自動投稿システムを作る
  - [x] GASで作る
  - [NEXT] 定期報告をツイートするものを作る

- [ ] 更新処理にて「言語」を消せるようにする

- [ ] ユーザー登録の「オートコンプリート」を直す
- [ ] 修正案をコメント機能を実装

- [ ] シェアボタンの文言を修正する
- [wip] 初回記事の作成
  - [ ] AI による記事作成を考えたい
  - [ ] 使い方記事
  - [ ] マークダウンの記事
- [ ] Joinの絞り込み


## リファクタリング

- [ ] comment-formとエディターにて同じ修正をする


## Next Features

- [ ] 質問機能
- [ ] トレンド機能
- [ ] 広告機能
- [ ] フォロー機能
- [ ] 企業ユーザーを作成する
- [ ] ダッシュボードの検索機能
  - [ ] debounce を入れる
- [ ] 通知 => 公式を追加する
  - [ ] お知らせにする

## デプロイ前

- [ ] 画像を削除する
- [ ] console.log が無いかを確認する
- [ ] username の unique をかける
- [ ] テーブルの制限をかける
  - 制限の Docs を整理する
- [ ] テストの作成


## 今後の対応タスク

- [x] 言語別検索
- [ ] ダッシュボードのページネーション
  - 作成
  - お気に入り
- [ ] コメントページのページネーション
- [ ] 通知一覧のページネーション
- [ ] アイコンの自由化
- [ ] コメントのページネーション
- [ ] タグ検索
- [ ] 文字数制限を広げる
- [ ] Google認証

## Hold

- [ ] 通知削除機能
  - comment_id, favorite_id が無い場合は削除していい
- [hold] Firebase analytics を入れる
- [ ] 時間のフォーマットを変える
- [ ] FileEditor
  - Makrdown の時だけ「マークダウンエディタ」にする
- [ ] コードタイプを「疑問」と「Topic」に分ける
- [ ] コードを Sticky にする
- [ ] ページネーション（一番最後）
- [ ] ctrl + s で保存する
- [ ] detail ページの CSS を直す
- [ ] いろんな言語に対応する
  - [ ] jsx, tsx に対応させる
- [ ] Hooks のテストを行う
- [ ] ページネーションを作る
- [ ] ファイル選択時
  - [hold] AutoFocus をつける
- [ ] Notification をトリガー化する
- [hold] ランダムに表示する
  - [ ] この記事もオススメ
- [ ] 話題のトピックセクションを作成する

## Done

- [x] パスワードの確認フォームを追加したい
- [x] 何が原因かを判定できるようにしたい
  - [x] メールアドレスかユーザー名が重複しています。変更してください。
- [x] ファイルスクロールを auto にしたい

- [x] マークダウン
  - [x] Twitter カード
  - [x] Twitter 表示
- [x] タイトル検索の実装
- [x] 利用規約を作成
  - 利用規約の同意を作成
- [x] コンテンツ表示箇所を変更
- [x] モーダルの位置を変更
- [x] お気に入り問題を解決する
- [x] 下部がクリックできない問題を解決する
- [x] お気に入りは 1 回のみにする
- [x] コメントに「Preview を入れたい」
- [x] ログイン時のユーザーを判定できるようにしたい
  - [x] サイドバーに名前を追加
- [x] 制限の確認
  - コード
    - タイトル
      - [x] 60 文字
    - 説明
      - [x] 15000 文字
  - ファイル
    - ファイル名
      - [x] 52 文字
    - ファイル内容
      - [x] 5000 文字
  - ユーザー
    - ユーザー名
      - [x] 30 文字
    - 自己紹介
      - [x] 160 文字
  - [x] Context を Hooks に入れる
- [x] プレビューにカードを形式を取り入れる
- [x] ダッシュボードにタグを入れる
- [x] プレビューにパディングを加える
- [x] ファイルエディターの改行を無くす
- [x] コメントなしの場合を一列にする（grid はずす）

- [x] ランダム記事表示機能
  - [x] 誰にでも閲覧チャンスを与えたい
  - [x] 本番にも入れる
    - CREATE VIEW random_codes_view AS SELECT \* FROM codes ORDER BY random();
- [x] エラー処理の共通化
- [x] 作成後にDashboardに反映されないバグを修正する
- [x] 画像アップロードボタンの実装
- [x] 画像制限処理
  - [x] APIのみ利用できるようにする
  - [x] 本番環境にservice keyを入れる
- [x] 画像アップロード機能を入れる
  - [x] コピペで入れられるようにする
  - [x] 本番にもバケットを作成する
  - [x] 画像アップロードのバグ修正
  - [x] ローディング処理を入れたい
  - [x] イメージボタンの配置
  - [x] 制限
    - [x] 1 ファイル 50MB まで
    - [x] 今月までの制限
- [x] 画像貼り付け処理
  - [x] カーソルの位置に入れる処理を追加する
- [x] ポストカードの修正
  - 現在表示されない
- [x] インデックスを貼る
- [x] 広告
  - [x] 書籍広告
    - [x] プレビューの検証をしたい
- [x] remark breaksを入れる
- [x] image paste後、フォーカス外れる現象を修正する
- [x] コードエディターの縦を広げる
- [x] OGPカードに「タイトル」をつける
- [x] userアイコン
- [x] ファイル引用機能の実装
- [x] 文字の大きさの調整をする
- [x] アイコンサイズを共通化する
- [x] ファイルアイコンを調整する
- [x] ランダムAPIを作成する
- [x] 削除機能の修正
- [x] 検索モーダルの表示修正
- [x] Sidebarのasync/awaitを解除
- [x] 良いね処理の修正
- [x] ファイル引用に「ファイル名」を表示させる
- [x] コメント欄での「画像添付」
- [x] ナイトモードの対応
- [x] usePaginateCodesを作成する
- [x] タグを5個にする
- [x] ツールページの作成
- [x] タグに注略を作成
  - [x] 注意ページを紐づける
- [x] 質問機能
  - [x] 質問ページを作成
- [x] 初心者用ページを作成
  - [x] 最初に読むべき記事を掲載
- [x] APIにタグをつける -> GASにつけた


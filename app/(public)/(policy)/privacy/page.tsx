...
PostCode（以下、本サービスと言います）は、提供するサービスにおける、ユーザーの個人情報の取扱いについて、以下のとおりプライバシーポリシー（以下、「本ポリシー」といいます。）を定めます。
...
本人から個人情報の開示を求められたときは、本人に対し、遅滞なくこれを開示します。ただし、開示することにより次のいずれかに該当する場合は、その全部または一部を開示しないこともあり、開示しない決定をした場合には、その旨を遅滞なく通知します。なお、個人情報の開示に際しては、1件あたり1000円の手数料を申し受けます。
...
メールアドレス：postcodeapps@gmail.com
...
const Page = () => {
  return (
    <div>
      <div className="max-w-5xl bg-white p-8 rounded-lg mx-auto mt-20 mb-32 min-h-[80vh]">
        <MarkdownPreviewer content={privacy} />
      </div>
      <TopFooter />
    </div>
  );
